...
<!-- Form Basic -->
    <div class="card mb-4">
        ...
        <div class="card-body">
        <form action="/dtpac-edit/{{ $pac->id }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="id_kec">Kecamatan</label>
                <select class="form-control" id="id_kec" name="id_kec">
                    <option value="{{ $pac->id_kec }}">{{ $pac->nama_kec->nama_kecamatan }}</option>
                    @foreach ($kec as $data)
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="nama_organisasi" name="nama_organisasi" value="{{ $pac->nama_organisasi }}" autocomplete="off" placeholder="Masukan Nama Organisasi">
            </div>
